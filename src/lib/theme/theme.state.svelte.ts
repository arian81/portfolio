...
export class Theme {
	...
	constructor(private options: ThemeOptions) {
		...
		$effect(() => {
			// Access resolvedTheme to create reactive dependency
	...
	private validateTheme(newTheme: string | undefined): void {
	...
	private setThemeStorage(theme: string): boolean {
		try {
 ...
 * Must be called within a component that is a child of SvelteTheme.
 *
 * @example
 * ```svelte
 * <script lang="ts">
...
export function useTheme(): Theme {
	...
	if (!theme) {
		throw new Error('Theme context not found. Make sure to wrap your app with <SvelteTheme>.');
	}
	return theme;
}
